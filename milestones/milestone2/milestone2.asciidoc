...
Aquí debemos ingresar entre otras cosas datos de contacto los cuales deberán ser verificados antes de permitirle a un desarrollador utilizar las funcionalidades de la plataforma que exijen este tipo de identificación. [Eze, que tipo de datos de contacto? Nuestra política es trabajar independiente de mecanismos estandares que dependen de elementos centralizados que pueden ser prohibidos u espiados, con lo cual no vamos a requerir emails, SMS, ni nada por el estilo. Luis] +
...
IMPORTANT: Pregunta técnica ¿Podemos incorporar el código nuevo a otro nodo sin tener que recompilar lo existente en todas las plataformas? [Respuesta NO. El código nuevo de los fragmentos van a una nueva versión del sistema. Luis]
...
[Eze, aca o cuando expliqués este módulo en detalle podés aclarar que el ambiente de ejecución del wallet factory "Wallet Factory Activity" heredará de la "Wallet Activity", en algun momento charlá de esto con Raul que esta reprogramando Android-core , Luis]

[Eze, otra cosa que hay que considerar es que exista un network service Wallet Factory como DAPP para poder compartir un proyecto entre varios dispositivos, de manera de poder probar en varios. Adicional a esto, si uno prueba con un emulador, como se hace para que la configuración que uno hace corriendo en un dispositivo no se pierda cuando se quiera probar en otro. Quizas pudieramos usar un repositorio github de puente para guardar las cosas mientras tanto, en vez de grabarlas en el telefono, cosa de que nos evitamos de que la wallet factory sea una DAPP solo por esto y ademas resolvemos el tema de probar desde distintos emuladores. Mientras apuntemos a un repo github, el proyecto quedaría automaticamente compartido. Luis]
...
Los fragmentos ya están instalados em el dispositivo, de manera que lo que falta es la WFP de la wallet. Para esto el Wallet Manager los obtiene a través del Network Service Wallet Resources. Del WFP se extrae la estructura de navegación, el Wallet Manager le inyecta la estructura al Wallet Runtime para que esté disponible a la hora que el usuario navegue a la nueva wallet dentro de la aplicación. + 
